...
//!
//! Usage:
//!   CHRONIKL_SIGNING_KEY=<hex> cargo run --example issue_license -- \
//!     --name "Acme Corp" --id "acme-001" --expires "2027-05-08"
...
fn main() {
    ...
    let private_hex =
        std::env::var("CHRONIKL_SIGNING_KEY").expect("CHRONIKL_SIGNING_KEY env var required");
    let private_bytes = hex::decode(&private_hex).expect("invalid hex in CHRONIKL_SIGNING_KEY");
